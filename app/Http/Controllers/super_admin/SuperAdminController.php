...
//    }
    ...
    {
        return view('backend.super_admin.count_detail_list.visitorcountlist');
    }
    ...
    public function getAllVisitor(Request $request)
    {
        ...
        $records = frontuserlogs::leftjoin('guestoruserid', 'front_user_logs.userorguestid', '=', 'guestoruserid.id')->leftjoin('users', 'users.id', '=', 'guestoruserid.user_id')->orderBy($columnName, $columnSortOrder)
            ->orderBy('front_user_logs.created_at', 'desc')->where('front_user_logs.status', 'homepage')
            ->where(function ($query) use ($searchValue) {
