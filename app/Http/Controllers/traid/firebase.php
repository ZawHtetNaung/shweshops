...
trait firebase
{
    ...
    {
        $items = \App\Usernoti::select('*')->join('items', 'user_noti.item_id', '=', 'items.id')->join('firebase', 'user_noti.receiver_user_id', '=', 'firebase.userid')->where('items.id', $itemid)->where('read_by_receiver', '!=', 1)->get();
        if (!empty($items)) {
            ...
            ])->post("https://fcm.googleapis.com/fcm/send", ['registration_ids' => $items->pluck('token'),
            ...
            if ($items->pluck('androidtoken')) {
                $res = \Illuminate\Support\Facades\Http::withHeaders([
                    'Authorization' => 'key=AAAAs3L7Dpk:APA91bHW_87I0M2LIU2dKyOhYbmkxcFFXH8xNasjpaUqNBtLqx73vmaQ8wcAIsl3T7b0tTF7fqLJDUCQFmqqnf1Duv6j4LaSl28dBaWyTxv5zuJUzTt4yl9HZjqZXm9VgCG2axNu_tSE',
                    'Content-Type' => 'application/json'
                ])->post("https://fcm.googleapis.com/fcm/send", ['registration_ids' => $items->pluck('androidtoken'),
