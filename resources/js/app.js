...
Vue.component(
    ...
    require("./components/frontend/discount/DiscountitemsForShopComponent.vue").default
);
    ...
    methods: {
        ...
        getfromidparent: async function (value) {
            ...
            if (localStorage.getItem(value.id + '_messages') === null) {
                ...
                var currentuserdata = await allfrommessagefunction.getcurrentchatshops(value.id, 0);
                //set shops messages to localstorage
                localStorage.setItem(value.id + '_messages', JSON.stringify(currentuserdata))

            } else {
                fromcache = true;
                // get data for user clicked shop
                var currentuserdata = JSON.parse(localStorage.getItem(value.id + '_messages'));
            ...
            temrevarray.map((d, index) => {
                if (typeof (temrevarray[index + 1]) !== 'undefined') {
                    if (temrevarray[index + 1].from_id == currentuserdata.data.data.messages[index].from_id) {
            ...
            if (currentuserdata.data.success) {
                if (value.info != 'frombuynow') {
                    ...
                    const setreadbyuser = allfrommessagefunction.setreadbyuser(value.id);
                    let tmplscl = JSON.parse(localStorage.getItem(window.userid + 'chatlist'));
                    ...
                    localStorage.setItem(window.userid + 'chatlist', JSON.stringify(tmplscl))
                ...
                localStorage.setItem(value.user_id + 'gettotalchatcountforuser', total_chat_count);
                app.$refs.chatlistref.getTotalCount();

                //specific count
                var count = JSON.parse(localStorage.getItem(value.user_id + 'getspecificcount'));
                count[froshop.shops_id] = 0;
                localStorage.setItem(value.user_id + 'getspecificcount', JSON.stringify(count));
        ...
        messengerbuttonclick: async function (checkauth, link) {
            let auth = checkauth;
            if (window.userid == undefined && auth != "1") {
                ...
                $(document).ready(function () {
                    ...
                    document.getElementById("loginbeforemessenger").value = "clickmessenger";
                });
        ...
        paymentbuttonclick: async function (checkauth,link) {
            ...
            if (window.userid == undefined && auth != "1") {
                ...
                $(document).ready(function () {
                    ...
                    document.getElementById("loginbeforepayment").value = "clickpayment";
                });
        ...
        buynowbuttonclick: async function (checkauth, to, message, type, name, shopdata, dt) {
            ...
            if (window.userid == undefined && auth != "1") {
                ...
                $(document).ready(function () {
                    ...
                    document.getElementById("loginbeforebuynow").value = "clickbuynow";
                });
                ...
                if (localStorage.getItem(to + '_messages') === null) {
                    await Promise.all(data.map(async (d) => {
                        const issend = await allfrommessagefunction.sendmessage(d);


                    }));
                    await app.$refs.chatlistref.passchildtoparent(to, '', "frombuynow");
                    ...
                    var tmplsd = JSON.parse(localStorage.getItem(to + '_messages'));
                    tmplsd.data.data.messages.unshift(data[0]);
                    tmplsd.data.data.messages.unshift(data[1]);
                    localStorage.setItem(to + '_messages', JSON.stringify(tmplsd));
                    await app.$refs.chatlistref.passchildtoparent(to, '', "frombuynow");
                    console.log(tmplsd)
                }
        ...
        requestpermission: async function () {
            ...
            if (checkalreadyhave == "none") {
                let subscribe = await ready.pushManager.subscribe({
                    ...
                    applicationServerKey: "BDp4y42lJL3ScdJdnjG-splBj-5_NIusKVEwfY07WSf750ZaDZM0tnkkM_XqDjICOvuGsMynIcH_6tUT7tfEY2U",
                });
        ...
        sendphonetoserver: function () {
            axios
                .post(
                    Vue.prototype.$hostname + "/" + "forgot_password", {emailorphone: this.phone}, {
                        header: {
        ...
        sendcodetoserver: function () {
            axios
                .post(
                    Vue.prototype.$hostname + "/" + "check_code", {emailorphone: this.phone, code: this.code}, {
                        header: {
                            "Content-Type": "multipart/form-data",
                        },
                    }
