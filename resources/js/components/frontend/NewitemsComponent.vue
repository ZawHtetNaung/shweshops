            ...
            axios
                ...
                .then((response) => {
                    ...
                    if (response.statusText == "OK") {
                        setTimeout(() => {
                          let setemptyonserver = (itemsemptyonserver, shopsempty) => {
                            if(itemsemptyonserver == 1 && this.newitemlatest === true){
                              // empty products for recent 2 months
                              this.current_shop_limit = 0;
                              this.newitemlimit = 0;
                              this.newitemlatest = false;
                              this.emptyonserver = 0;
                            } else if (itemsemptyonserver == 0 && shopsempty == 1 && this.newitemlatest === true) {
                              // there are more products on server but shop list empty (have to restart shop list) for recent 2 months
                              this.current_shop_limit = 0;
                              this.newitemlimit += 4;
                            } else if (itemsemptyonserver == 0 && shopsempty == 0 && this.newitemlatest === true) {
                              // there are more products on server and shops list update
                              this.current_shop_limit = response.data['current_shop_limit'];
                            }
                            else if(itemsemptyonserver == 1 && this.newitemlatest === false){
                              // empty on server all done
                              this.emptyonserver = 1;
                              this.emptyonservernew = 1;
                            } else if (itemsemptyonserver == 0 && shopsempty == 1 && this.newitemlatest === false) {
                              // there are more products but shop list empty (have to restart shop list) for recent 2 months before

                              this.current_shop_limit = 0;
                              this.newitemlimit += 4;
                            } else if (itemsemptyonserver == 0 && shopsempty == 0 && this.newitemlatest === false) {
                          ...
                          async function tosetdata() {
                            await setemptyonserver(
                                response.data['itemsemptyonserver'], response.data['shopsempty']
                            );
