...
<div class="container-fluid px-1">
    ...
    >
      <div>
        <div class="state-dropdown">
          ...
          <label for="toggle2" class="animate">{{this.state_label ? this.state_label +'ဆိုင်များ' : 'ဆိုင်များအားလုံး'}} <i class="fas fa-chevron-down"></i></label>
          <ul class="animate p-0">
            ...
            <li class="animate" 
              ...
              :value="state.id" 
              :key="state.key"
              @click="filterShop(state.myan_name, state.id)">
              {{state.myan_name}}ဆိုင်များ
            </li>
          </ul>
        </div>
      </div>
      <button class="p-2 filter-button d-block d-md-none" @click="showFilter"><i class="fa-solid fa-sliders-h"></i> Filter</button>
    </div>
      ...
      >
        <div>
            <article class="post-wrapper">
                <div id="cato_slide" class="post-img sop-cato-img">
                    <input
                        type="checkbox"
                        value="all"
                        id="allshop"
                        ...
                        class="shop-icon"
                    />
                    <label for="allshop" class="px-1 px-md-2 mb-0">
                        <img
                            :src="
                                host +
                                '/test/img/allshops.png'
                            "
            ...
            <article class="post-wrapper">
                <div id="cato_slide" class="post-img sop-cato-img">
                    <input
                        ...
                        class="shop-icon"
                    />
                    <label :for="f.id" class="px-1 px-md-2 mb-0">
                        <img
                            :src="
                                host +
                                '/images/logo/mid/' +
  ...
  <div class="row container-fluid p-0 m-0">
    ...
    <div class="d-none d-md-block col-12 col-md-3 pe-md-3 pe-lg-5 filter-container">
      <div class="filter-sticky">
        <button @click="hideFilter" class="d-block d-md-none filter-close-button"><i class='fa fa-close'></i></button>
        ...
        <div class="col-12 mb-2 mt-2 sn-radio-filter">
          ...
          <radio name="product_type" value="all" v-model="main_product_type" @input="getMainProductType">
              All
          </radio>
          <radio name="product_type" value="အဝါရောင်ရွှေ" v-model="main_product_type" @input="getMainProductType">
              Gold (ရွှေ)
          </radio>
          <radio name="product_type" value="အဖြူရောင်ရွှေ" v-model="main_product_type" @input="getMainProductType">
              Platinum (ပလက်တီနမ်)
          </radio>
          <radio name="product_type" value="၁၂ပဲရည်" v-model="main_product_type" @input="getMainProductType">
              18k (၁၂ပဲရည်)
          </radio>
        </div>
        <div class="col-12 mb-2 mt-2">
            ...
            <button @click="catSeeMore()" class="d-block" id="cat-more-btn">See more...</button>
        ...
        <div class="col-12 mb-2 mt-2 gems-container">
            ...
            <span
              ...
              class="mm_font position-relative mb-3 d-block">
                <checkbox
                  ...
                  @input="setGems()"
                >
                  <span class="gem-checked-icon position-absolute"><i class="fa-solid fa-check"></i></span>
                  <span class="gem-img">
                    <img :src="host + '/images/icons/included_gems/' + gem[1]" />
                  </span>
                  {{ gem[0] }}
                </checkbox>
            </span>
            ...
            <button @click="gemsSeeMore()" class="d-block" id="gems-more-btn">See more...</button>
        </div>

        <div class="col-12 mb-2 mt-2 sn-radio-filter">
          <label class="sn-label" for="">Gender</label>
          <radio name="gender" value="all" v-model="selectedgender" @input="getGender">
              All
          </radio>
          <radio name="gender" value="Women" v-model="selectedgender" @input="getGender">
              Women
          </radio>
          <radio name="gender" value="Men" v-model="selectedgender" @input="getGender">
              Men
          </radio>
          <radio name="gender" value="Kid" v-model="selectedgender" @input="getGender">
              Kid
          </radio>
          <radio name="gender" value="Couple" v-model="selectedgender" @input="getGender">
              Couple
          </radio>
          <radio name="gender" value="UniSex" v-model="selectedgender" @input="getGender">
              Uni Sex
          </radio>
        </div>
        <button class="d-block d-md-none filter-save" @click="hideFilter">Save</button>
      </div>
    </div>
  ...
  methods: {
    ...
    getMainProductType: function () {
        if (this.main_product_type == "all") {
            this.gold_quality = "all";
            this.gold_colour = "all";
        } else if (this.main_product_type == "၁၂ပဲရည်") {
            this.gold_quality = this.main_product_type;
            this.gold_colour = "all";
        } else {
    ...
    getdatafromserver_bysort: function () {
      ...
      axios
        ...
        .then((response) => {
          ...
          console.log("byshop response", this.byshop);
        });
...
<style scoped>
  ...
  .state-dropdown label, .state-dropdown ul li{
    display: block;
    width: 200px;
    background: #fff;
    padding: 10px 10px;
