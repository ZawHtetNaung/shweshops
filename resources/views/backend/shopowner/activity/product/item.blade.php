        ...
        <!-- Main content -->
            <section class="content pt-3">
                <div class="sn-tab-panel">
                    <ul>
                      <a class="active-panel" href="{{route('backside.shop_owner.so_activity.p_product')}}">
                            ...
                            <div class="sn-table-list-wrapper">
                                ...
                                <div class="card shadow-none border-0 rounded-5 pb-2">
                                    ...
                                    <div class="card-header border-0">
                                        <h2>Panel Items Activity @include('backend.shopowner.toottips')</h2>
                                        <p>Check your store’s panel users activity</p>
                                    </div>
                                    <div class="d-flex justify-content-end my-3 align-items-center">
                                      <div class="form-group mr-md-2">
                                        <fieldset>
                                          <legend>From Date</legend>
                                          <input type="text" id='search_fromdate_itemact' class="itemactdatepicker form-control" placeholder='Choose date' autocomplete="off"/>
                                        </fieldset>
                                      </div>
                                      <div class="form-group mr-md-2">
                                        <fieldset>
                                          <legend>To Date</legend>
                                          <input type="text" id='search_todate_itemact' class="itemactdatepicker form-control" placeholder='Choose date' autocomplete="off"/>
                                        </fieldset>
                                      </div>
                                      <div class="pr-md-4">
                                        <input type='button' id="itemact_search_button" value="Search" class="btn bg-info">
                                      </div>
                                    </div>
                                    <div class="table-responsive">
                                        <table id="itemsActivityTable" class="table table-borderless">
                                            <thead>
                                            <tr>
                                                <th>ID</th>
                                                <th>Product Code</th>
...
@push('scripts')
    <script>
      $(document).ready(function() {
          // Item Activity Table
          var itemsActivityTable = $('#itemsActivityTable').DataTable({
              processing: true,
              serverSide: true,
              ajax: {
                  "url": "{{ route('backside.shop_owner.items.getitems_activity_log') }}",
                  'data': function (data) {
                      // Read values
                      var from_date = $('#search_fromdate_itemact').val() ? $('#search_fromdate_itemact').val() + " 00:00:00" : null;
                      var to_date = $('#search_todate_itemact').val() ? $('#search_todate_itemact').val() + " 23:59:59" : null;
