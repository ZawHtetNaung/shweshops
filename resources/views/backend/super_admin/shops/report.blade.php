    ...
    <div class="row">
        <div class="col-12 p-3">
            <div class="d-flex justify-content-end align-items-center">
                ...
                <div class="ml-3 p-1">
                    <fieldset>
                        ...
                        <input type="text" id='search_todate_shop' value="" class="shopdatepicker form-control"
            ...
            <div class="w-100  d-flex justify-content-center">
                <button id="btn_convert" class="btn btn-warning">Download PNG <i class="fas fa-download"></i></button>
            </div>
        </div>
    </div>
            ...
            <div class="report-card" id="myHtml">
                <div class="report-card-header gradient p-4">
                    <div class="report-card-title mb-2">
                        <img src="{{url('images/logo/'.$shopid->shop_logo)}}" alt="logo" class="rounded-circle ">
                ...
                <div class="report-card-body">
                    <div class="report-card-body-inner">
                        <div class="report-card-body-inner-title d-flex justify-content-between align-items-center p-3">
                            <h3>Features</h3>
                            <h3>Counts</h3>
                        </div>
                        <h6 class="text-uppercase text-center font-weight-bolder mt-3">Shwe Shops</h6>
                        <div class="report-card-count-lists">
                            <span>အသုံးပြုသူစုစုပေါင်း</span><span id="totalUsers">{{ $total_user_count }} ဦး</span>
                        </div>
                        <div class="report-card-count-lists">
                            <span>Products အရေအတွက်စုစုပေါင်း</span><span id="totalProducts">{{ $total_products_count }} ခု</span>
                        </div>
                        <div class="report-card-count-lists">
                            <span>ဆိုင်စုစုပေါင်းအရေအတွက်</span><span id="totalUsers">{{ $shop_counts }} ဆိုင်</span>
                        </div>
                        <div class="report-card-count-lists">
                        ...
                        @if( count($products_count_setting) != 0)
                            <div class="report-card-count-lists">
                                <span>Total Products အရေအတွက် (All Time)</span><span id="totalitems">{{ count($items) }} ခု</span>
                            </div>
                        @endif
                    ...
                    <!-- Item counts -->
                        @if( count($products_count_setting) != 0)
                            <div class="report-card-count-lists">
                                <span> Products အရေအတွက် </span><span
                                    id="shopItemCounts">{{ count($items) }} ခု</span>
                            </div>
                        @endif
                    ...
                    <!-- Shop View Counts -->
                        @if (count($shops_view_count_setting) != 0 )
                            <div class="report-card-count-lists">
                                <span><span
                                        class="for_date_shop"></span> ဆိုင်ကိုကြည့်ရှုထားသော  customer အရေအတွက် </span><span
                                    id="shopViewCounts">{{ count($shopview) }} ယောက်</span>
                            </div>
                        @endif
                        ...
                        @if (count($unique_product_click_count_setting) != 0)
                            <div class="report-card-count-lists">
                                <span> Products များကိုကြည့်ရှုထားသော  customer အသစ်အရေအတွက်</span><span
                                    id="shopProductViewUniqueCounts">{{ count($unique_productclick) }} ယောက်</span>
                            </div>
                        @endif
    ...
    $(document).ready(function () {
        ...
        $("#btn_convert").on('click', function () {
            html2canvas(document.getElementById("myHtml"),
                ...
                anchorTag.download = `{{$shopid->shop_name}} {{ date('F')}} Report.jpg`;
