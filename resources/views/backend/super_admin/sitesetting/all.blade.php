        ...
        <ul class="list-group col-12 col-md-6 mx-1 mx-md-4">
          @foreach ($sitesettings as $sitesetting)
            <li class="list-group-item d-flex justify-content-between align-items-center">
              <div>{{ $sitesetting->name }}  feature is <span id="toggleText{{ $sitesetting->id }}"></span> </div>
              <div class="custom-control custom-switch">
                <input type="hidden" id="{{ $sitesetting->id }}">
                <input type="checkbox" name="settings" class="custom-control-input" id="settingToggle{{ $sitesetting->id }}" name='machine_state'>
                <label class="custom-control-label" id="statusText" for="settingToggle{{ $sitesetting->id }}"></label>
              </div>
            </li>
          @endforeach
        </ul>
